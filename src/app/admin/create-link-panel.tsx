...
export function CreateLinkPanel({
  ...
  const [open, setOpen] = useState(Boolean(error || created));
    ...
    <div className="mb-4">
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="inline-flex h-10 items-center justify-center rounded-sm bg-accent px-4 text-[14px] font-semibold text-white hover:bg-accent-hover"
        >
          + 새 링크 만들기
        </button>
      </div>
      ...
      {open && (
        <form
          action={createLink}
          className="mb-4 rounded-md border border-border bg-surface p-6 shadow-sm"
        >
          {error && (
